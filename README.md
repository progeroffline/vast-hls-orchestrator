...
### Явный attach SSH-ключа к instance, с проверкой

Аккаунтные SSH-ключи (Console → Keys) по документации Vast.ai должны подставляться в любой новый instance автоматически, но при нескольких зарегистрированных ключах на аккаунте Vast может инжектировать не все — именно так проявляется ситуация "через консоль заходит, через API/оркестратор нет" при полностью корректном ключе и аккаунте. Поэтому сразу после успешного create orchestrator дополнительно и явно привязывает ключ к instance:
...
Публичная часть берётся из `<--ssh-key>.pub`, а если такого файла нет — извлекается из приватного ключа через `ssh-keygen -y -f <--ssh-key>` (ключ должен быть без passphrase, как и для всех остальных non-interactive SSH-операций).

Важно: ответ этого запроса ненадёжен как единственный источник истины. `{"success": false, "msg": "SSH key already associated with instance."}` означает лишь, что **этот конкретный** публичный ключ (посимвольно) уже числится у Vast для instance — но не гарантирует, что именно он реально попал в `authorized_keys` контейнера. Поэтому сразу после attach orchestrator дополнительно вызывает:

```http
GET /api/v0/instances/<instance_id>/ssh
```

и сверяет, есть ли наш ключ в возвращённом списке `ssh_keys`, посимвольно. Если да — provisioning продолжается. Если нет — pipeline **немедленно падает** с понятной ошибкой вместо того, чтобы тихо ждать `--boot-timeout`/`--ssh-reconnect-timeout` ради соединения, которое заведомо не может состояться.
